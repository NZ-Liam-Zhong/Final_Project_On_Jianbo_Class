...
For language features, we retrieve hidden states from the language model backbone:

\begin{lstlisting}[caption={Extracting language hidden states from OpenVLA.}]
def openvla_get_language_layers(model, inputs) -> List[torch.Tensor]:
    """Retrieve language hidden states layers for OpenVLA.
    
    Returns a tuple/list where each entry is [B, T, H] tensor.
    """
    with torch.no_grad():
        if inputs.get("pixel_values") is not None:
            target_dtype = next(model.parameters()).dtype
            inputs["pixel_values"] = inputs["pixel_values"].to(
                model.device, dtype=target_dtype)
    ...
    raise RuntimeError("Could not retrieve language hidden states.")
\end{lstlisting}

\subsection{Gradient Attribution for Clusters}
\label{app:gradient_attribution}

To compute which feature channels contribute most to a given NCut cluster (as discussed in Section 4.6), we backpropagate through the k-way eigenvector assignments:

\begin{lstlisting}[caption={Computing channel gradients for cluster attribution.}]
def channel_gradient_from_cluster(
